"""
Mock supply chain data generator — backend copy.
Same as ML team's generator; produces the shared seed JSON.
...
def _sup(tier, name, category, country, region, lat, lon):
    return {
        "id": str(uuid.uuid4()), "name": name, "tier": tier,
...
def _edge(src_id, tgt_id, category):
    return {
        "id": str(uuid.uuid4()),
        "source_supplier_id": src_id, "target_supplier_id": tgt_id,
...
def _event(supplier, scenario="random"):
    base = {
        "tsmc_shutdown": {"disruption_type":"factory_shutdown","severity":0.93,
                          "description":"Major semiconductor fab shutdown — 45% of global chip supply affected",
                          "affected_capacity_pct":45.0},
        "rare_earth_ban": {"disruption_type":"geopolitical","severity":0.78,
                           "description":"Export controls on rare earth metals — 6-month disruption",
                           "affected_capacity_pct":30.0},
        "random": {"disruption_type":random.choice(["factory_shutdown","natural_disaster",
                                                     "logistics_delay","quality_issue","capacity_constraint"]),
                   "severity":round(random.uniform(0.4, 0.9), 2),
                   "description":f"Disruption at {supplier['name']} affecting supply chain",
                   "affected_capacity_pct":round(random.uniform(15, 60), 1)},
    }.get(scenario, {})
    return {
        "id": str(uuid.uuid4()), "supplier_id": supplier["id"],
        "source": "mock_generator", "country": supplier["country"],
        "category": supplier["category"], "status": "active",
        "estimated_revenue_at_risk_usd": round(random.uniform(1e6, 5e8), 2),
        "downstream_affected": "[]", "resolved_at": None,
...
def generate_all() -> dict:
    ...
    for tier, entries in COMPANIES.items():
        ...
        for entry in entries:
            s = _sup(tier, *entry)
            suppliers.append(s); tier_map[tier].append(s)
    ...
    events = [
        _event(tier_map["tier_3"][0], "tsmc_shutdown"),
        _event(tier_map["tier_3"][1], "rare_earth_ban"),
        *[_event(s) for s in tier_map["tier_3"][2:5]],
    ]

    return {
        "suppliers": suppliers, "edges": edges, "events": events,
        "stats": {"total_suppliers":len(suppliers),"total_edges":len(edges),
